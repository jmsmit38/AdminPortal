...
const dataGenerator = (index) => {
    ...
    return {
        ...
        "Solar": referenceValue,
        "Pest": _.random(1200, 2200),
        "Roofing": referenceValue - _.random(halfedRefVal, halfedRefVal * 1.1),
    };
}
...
export const AudienceMetricsChart = ({ height, className }) => (
    ...
    >
        <ComposedChart data={data}
            ...
            <Tooltip />
            <Legend />
            <Bar dataKey='Solar' barSize={5} fill={colors['400']} />
            <Area dataKey='Pest' fill={colors['purple-02']} stroke={colors['purple']} activeDot={null} />
            <Area dataKey='Roofing' fill={colors['primary-04']} stroke={colors['primary']} activeDot={{ r: 5 }} dot={generateDot} />
        </ComposedChart>
    </ResponsiveContainer>
);
